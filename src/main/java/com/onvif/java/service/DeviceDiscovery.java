...
public class DeviceDiscovery {
  ...
  public static final String WS_DISCOVERY_ADDRESS_I_PV4 = "239.255.255.250";

  /** IPv6 not supported yet. set enableIPv6 to true for testing if you need IP6 discovery. */
  public static final boolean ENABLEI_PV6 = false;

  public static final String WS_DISCOVERY_ADDRESS_I_PV6 = "[FF02::C]";
  ...
  public static Collection<URL> discoverWsDevicesAsUrls(String regexpProtocol, String regexpPath) {
    ...
    for (String key : discoverWsDevices()) {
      try {
        ...
        // ignore ip6 hosts
        if (ok && !ENABLEI_PV6 && url.getHost().startsWith("[")) {
          ok = false;
        }
        if (ok) {
          urls.add(url);
        ...
        while (interfaces.hasMoreElements()) {
          ...
          if (!anInterface.isLoopback()) {
            ...
            for (InterfaceAddress address : interfaceAddresses) {
              ...
              if (!ENABLEI_PV6 && address.getAddress() instanceof Inet6Address) {
                continue;
              }
                ...
                if (address instanceof Inet4Address) {
                  server.send(
                      new DatagramPacket(
                          ...
                          probe.length(),
                          InetAddress.getByName(WS_DISCOVERY_ADDRESS_I_PV4),
                          WS_DISCOVERY_PORT));
                } else {
                    ...
                    if (ENABLEI_PV6) {
                      server.send(
                          new DatagramPacket(
                              probe.getBytes(StandardCharsets.UTF_8),
                              probe.length(),
                              InetAddress.getByName(WS_DISCOVERY_ADDRESS_I_PV6),
                              WS_DISCOVERY_PORT));
                    }
                  } else {
